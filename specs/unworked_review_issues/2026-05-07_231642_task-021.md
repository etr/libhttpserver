...
4. [x] **security-reviewer** | `src/webserver.cpp:1519` | insecure-design
   ...
   *Deferred:* Superseded and tracked in 2nd-pass review items 6 and 24 (2026-05-07_232016_task-021.md). The loop now lives in `detail/format_allow_header` in method_utils.hpp; structural refactor deferred to dedicated task.

5. [x] **code-simplifier** | `src/httpserver/detail/modded_request.hpp:49` | naming
   ...
   *Fixed:* Removed `TASK-021:` / `TASK-021` prefixes from modded_request.hpp, webserver_impl_dispatch.hpp, webserver_request.cpp, webserver_routes.cpp, method_utils.hpp, http_method.hpp, http_resource.hpp, test/unit/http_resource_test.cpp, test/unit/webserver_route_test.cpp, and test/integ/basic.cpp.

6. [x] **code-simplifier** | `src/httpserver/http_resource.hpp:214` | code-structure
   ...
   *Fixed:* Replaced the `TASK-021 acceptance:` block comment with a plain rationale explaining the size constraint and measured growth intent. Removed PRD-REQ references.

7. [x] **security-reviewer** | `src/webserver.cpp:1519` | insecure-design
   (Duplicate of item 4 above — same finding categorised as separate minor.)
   *Deferred:* Duplicate of item 4; deferred with it.
