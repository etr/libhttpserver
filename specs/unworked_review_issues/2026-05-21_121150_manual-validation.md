...
6. [ ] **code-simplifier** | `src/webserver.cpp:592` | code-structure
   ...
   *Status:* deferred — make_route_entry helper not yet extracted; repeated route_entry{methods, std::move(shim), false} pattern still present in webserver_routes.cpp.

7. [ ] **performance-reviewer** | `src/webserver.cpp:1874` | memory-allocation
   ...
   *Recommendation:* Accept the path as std::string_view and return early with a string_view (or a small inline buffer) when no transformation is needed. For the common case where the path already starts with '/' and has no trailing '/', the function can return its argument unchanged without allocating. A simple approach: keep the std::string fallback only for the two transformation branches (prepend '/' or strip trailing '/'), which are rare in practice.
   *Status:* deferred — canonicalize_lookup_path in webserver_dispatch.cpp still returns std::string on every call; string_view optimization not yet applied.

8. [ ] **performance-reviewer** | `src/webserver.cpp:1976` | memory-allocation
   ...
   *Recommendation:* Pre-normalize the auth_skip_paths entries once at registration time (when the webserver is built via create_webserver) so normalize_path is never called per request. The per-request call in should_skip_auth can then compare the raw standardized_url directly against the pre-normalized skip list. If normalization must remain per-request, preallocate segments with segments.reserve(count('/')) to avoid repeated reallocation.
   *Status:* deferred — normalize_path still runs per request in should_skip_auth (webserver_request.cpp:140); registration-time pre-normalization not yet implemented.

9. [ ] **performance-reviewer** | `src/webserver.cpp:2347` | missing-caching
   ...
   *Status:* deferred — serialize_allow_methods (webserver_dispatch.cpp:363) is now a shared helper (TASK-048) but still builds string on every call; route_entry caching not yet implemented.

10. [ ] **security-reviewer** | `.github/workflows/verify-build.yml:454` | supply-chain
    ...
    *Recommendation:* Add a sha256sum check immediately after the curl download, mirroring the pattern used for libmicrohttpd: `echo "<expected-sha256>  /tmp/pmd.zip" | sha256sum -c`. Pin the expected digest in the workflow file (or in a companion checksums file) and update it with each PMD version bump. The hash for PMD 7.24.0 should be obtained from the official PMD release notes or SBOM and committed alongside this change.
    *Status:* deferred — no sha256sum check added for pmd.zip in verify-build.yml; a TODO comment was added in a prior commit but the actual hash check is not yet implemented.
...
13. [ ] **architecture-alignment-checker** | `scripts/check-complexity.sh:1` | pattern-violation
...
14. [ ] **architecture-alignment-checker** | `src/httpserver/detail/radix_tree.hpp:117` | adr-violation
...
15. [ ] **architecture-alignment-checker** | `src/httpserver/detail/webserver_impl.hpp:370` | adr-violation
...
16. [ ] **architecture-alignment-checker** | `src/httpserver/http_utils.hpp:440` | pattern-violation
...
17. [ ] **code-quality-reviewer** | `src/http_utils.cpp:412` | code-readability
...
18. [ ] **code-quality-reviewer** | `src/http_utils.cpp:430` | test-coverage
...
19. [ ] **code-quality-reviewer** | `src/httpserver/detail/http_endpoint.hpp:192` | code-readability
...
20. [ ] **code-quality-reviewer** | `src/httpserver/http_method.hpp:127` | code-elegance
...
21. [ ] **code-quality-reviewer** | `src/webserver.cpp:806` | code-readability
...
24. [ ] **code-quality-reviewer** | `test/unit/webserver_register_path_prefix_test.cpp:399` | test-coverage
...
29. [ ] **code-simplifier** | `src/http_request.cpp:506` | code-structure
...
32. [ ] **code-simplifier** | `src/webserver.cpp:315` | code-structure
...
33. [ ] **code-simplifier** | `src/webserver.cpp:951` | code-structure
...
34. [ ] **housekeeper** | `/Users/etr/progs/libhttpserver/CONTRIBUTING.md:130` | documentation-stale
...
35. [ ] **housekeeper** | `/Users/etr/progs/libhttpserver/README.md:88` | documentation-stale
...
36. [ ] **housekeeper** | `/Users/etr/progs/libhttpserver/specs/architecture/09-testing.md:null` | architecture-not-updated
...
37. [ ] **performance-reviewer** | `src/http_utils.cpp:564` | missing-caching
...
38. [ ] **performance-reviewer** | `src/httpserver/detail/radix_tree.hpp:104` | memory-allocation
...
39. [ ] **performance-reviewer** | `src/webserver.cpp:1619` | memory-allocation
...
40. [ ] **performance-reviewer** | `src/webserver.cpp:1946` | memory-allocation
...
41. [ ] **performance-reviewer** | `src/webserver.cpp:2092` | memory-allocation
...
42. [ ] **performance-reviewer** | `src/webserver.cpp:2523` | memory-allocation
...
43. [ ] **security-reviewer** | `/Users/etr/progs/libhttpserver/src/http_request.cpp:1161` | sensitive-data-logging
...
44. [ ] **security-reviewer** | `/Users/etr/progs/libhttpserver/src/http_request.cpp:537` | cryptographic
...
45. [ ] **security-reviewer** | `/Users/etr/progs/libhttpserver/src/webserver.cpp:1807` | error-handling
...
46. [ ] **security-reviewer** | `/Users/etr/progs/libhttpserver/src/webserver.cpp:1995` | input-validation
...
47. [ ] **spec-alignment-checker** | `/Users/etr/progs/libhttpserver/specs/product_specs.md:238` | specification-gap
...
48. [ ] **spec-alignment-checker** | `/Users/etr/progs/libhttpserver/specs/product_specs.md:null` | specification-gap
...
49. [ ] **spec-alignment-checker** | `/Users/etr/progs/libhttpserver/specs/product_specs.md:null` | specification-gap
...
50. [ ] **test-quality-reviewer** | `scripts/check-complexity.sh:1` | missing-test
...
51. [ ] **test-quality-reviewer** | `scripts/check-duplication.sh:1` | missing-test
...
52. [ ] **test-quality-reviewer** | `src/http_utils.cpp:394` | missing-test
...
55. [ ] **test-quality-reviewer** | `test/unit/webserver_on_methods_test.cpp:438` | naming-convention
...
56. [ ] **test-quality-reviewer** | `test/unit/webserver_on_methods_test.cpp:522` | multiple-concerns
