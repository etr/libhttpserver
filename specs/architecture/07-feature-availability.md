...
| `HAVE_DAUTH` | Digest-auth disabled | `get_digested_user` returns empty; `check_digest_auth` returns a sentinel result; features().digest_auth == false; `create_webserver::digest_auth(true)` throws `feature_unavailable` at `webserver` construction time (consistent with `HAVE_BAUTH` and `HAVE_GNUTLS`). Response-side: `http_response::unauthorized(digest_challenge)` (TASK-062) is always declared regardless of `HAVE_DAUTH` (PRD-FLG-REQ-001: public declarations must not be conditionally compiled), but on a `HAVE_DAUTH`-off build the call throws `feature_unavailable("digest_auth", "HAVE_DAUTH")` instead of constructing a response |
