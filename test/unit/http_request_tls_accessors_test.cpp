...
// Compile-time guarantees of http_request's high-level GnuTLS
// accessor surface. We assert the structural invariants of that surface:
//
...
//
//   3. Return-type lockdowns for every signature in the accessor surface
...
//      includes. We rely on the standard staged-install grep
...
// only kicks in if the member exists. The member has been removed
// from the v2 surface, so the primary stays selected and value remains false.
